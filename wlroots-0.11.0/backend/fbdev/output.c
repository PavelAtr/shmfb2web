...
static bool create_fbo(struct wlr_fbdev_output *output, uint32_t width, uint32_t height) {
	...
	if (shmfb == NULL) {
	    ...
	    char* envxres = getenv("WLR_SHM_WIDTH");
	    char* envyres = getenv("WLR_SHM_HEIGHT");
	    scr_var.xres = (envxres == NULL) ?  1024 : atoi(envxres);
	    scr_var.yres = (envyres == NULL) ?  768 : atoi(envyres);
	    scr_var.xoffset = 0;
	    scr_var.yoffset = 0;
	    scr_var.bits_per_pixel = 32;
	    scr_fix.line_length = scr_var.xres * scr_var.bits_per_pixel / 8;
	    if (width && height) {
		scr_var.xres = width;
		scr_var.yres = height;
	    }
	    fbmem_size = scr_var.xres * scr_var.yres * sizeof(uint32_t);
	    int r = ftruncate(fd, fbmem_size);
	    if (r != 0) {
		wlr_log(WLR_ERROR, "shmfb ftruncate failed");
		return false;
	    }
	    output->fbmem = mmap(NULL, fbmem_size, PROT_WRITE, MAP_SHARED, fd, 0);
	    if (output->fbmem == MAP_FAILED) {
		wlr_log(WLR_ERROR, "shmfb mmap failed");
		return false;
	    }
	}
