...
char* lws_callback_recieve2(struct lws *wsi, enum lws_callback_reasons reason, void *user, void *in, size_t len) {
    ...
    memcpy( &received_payload2.data[LWS_SEND_BUFFER_PRE_PADDING], in, len );
    received_payload2.len = len;
    memcpy(receivestr2, &received_payload2.data[LWS_SEND_BUFFER_PRE_PADDING], received_payload2.len);
    receivestr2[received_payload2.len] = '\0';
    lws_callback_on_writable_all_protocol( lws_get_context( wsi ), lws_get_protocol( wsi ) );
...
int lws_callback_server_writable2(struct lws *wsi, enum lws_callback_reasons reason, void *user, void *in, size_t len, char* receivestr) {
    ...
    if(strstr(receivestr2, "input: ") != NULL)
    {
	...
	sprintf(str, "%lu %s", info.uptime * 1000 + tv.tv_usec / 1000, receivestr2);
